...
class Orbit extends React.Component {
    constructor(props) {
        ...
        this.state = {
            orbit: {
                transform: "rotate(" + getRandomNumber(0, 360) + "deg)",
                animationDuration: getRandomNumber(3, 60) + "s"
            },
            satellite: {
                left: getRandomNumber(0, 250) + "px",
                top: getRandomNumber(0, 250) + "px"
            }
        }
    }
    ...
    render() {
        return (
            <div className="orbit" style={this.state.orbit}>
                <div className="orbit__satellite" style={this.state.satellite}>
...
class Scene extends React.Component {
    ...
    renderOrbit(i) {
        this.state.list = Array.from({length: i}, () => null);;
        console.log(this.state.list);
        let res = this.state.list.map((item, index) => {
            return <Orbit key={index} name={index}/>;
        });
    ...
    render() {
        ...
        return (
            <div className="container">
                ...
                {this.renderOrbit(getRandomNumber(min, max))}
            </div>
        )
    }
}
